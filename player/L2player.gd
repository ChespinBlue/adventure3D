...
func _input(event):
	...
	if event.is_action_pressed("player1left"):
		lefpres = true
	if event.is_action_released("player1left"):
		lefpres = false
	if event.is_action_pressed("player1right"):
		rigpres = true
	if event.is_action_released("player1right"):
		rigpres = false
	if event.is_action_pressed("player1up"):
		uppres = true
	if event.is_action_released("player1up"):
		uppres = false
	if event.is_action_pressed("player1down"):
		dowpres = true
	if event.is_action_released("player1down"):
		dowpres = false
		
	if event.is_action_pressed("player1shoot"):
		shoot()
	if Input.is_action_just_pressed("player1five"):
...
func _physics_process(delta):
	...
	var input_dir = Input.get_vector("player1leftwalk", "player1rightwalk", "player1forwardwalk", "player1backwalk")
...
func _on_health_p_1_health_deplete():
	death()
